...
impl PcscReader {
    ...
    pub fn new() -> Result<Self> {
        let context =
            Context::establish(Scope::User).context("Failed to establish PCSC context")?;
    ...
    pub fn list_readers(&self) -> Result<Vec<ReaderInfo>> {
        ...
        for reader_name in readers {
            ...
            let (is_connected, atr) = self
                .get_reader_status(&reader_name_str)
...
mod tests {
    ...
    fn test_reader_info_with_atr() {
        let atr = vec![
            0x3B, 0xAC, 0x00, 0x40, 0x2A, 0x00, 0x12, 0x25, 0x00, 0x64, 0x80, 0x00, 0x03, 0x10,
            0x00, 0x90, 0x00,
        ];
        let reader_info = ReaderInfo {
